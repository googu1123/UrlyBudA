...
<body class="skin-blue">
    ...
    <div class="wrapper row-offcanvas row-offcanvas-left">
        ...
        <aside class="right-side">                
            ...
            <!-- Main content -->
            <form action="campaignList.tsis" method="post"  id="mainForm">
	            ...
	            <section class="content">
		                            ...
		                            <dd>
		                                <div class="form-inline">
		                                    <select class="form-control" id="use_yn" name="use_yn">
		                                        ...
		                                        <option value="Y" <c:if test="${use_yn eq 'Y' }">selected="selected"</c:if>>사용</option>
		                                        <option value="N" <c:if test="${use_yn eq 'N' }">selected="selected"</c:if>>중지</option>
		                                    </select>
		                                </div>
		                            </dd>
	                ...
	                <div class="list_bot">
	                                	...
	                                	<c:choose>
	                                		<c:when test="${fn:length(campaignList) > 0 }">
	                                			<c:forEach var="campaign" items="${campaignList }" varStatus="status">
	                                				<tr data-code="${campaign.seq }" data-pidx="${campaign.seq }">
				                                        <td><input type="checkbox" class="ichk chk_code"></td>
				                                        <td>${campaign.aidx }</td>
				                                        <td>${campaign.subject }</td>
				                                        <td>${campaign.wrtdate }</td>
				                                        <td>${campaign.link_limit_day }</td>
				                                        <td>${campaign.link_cnt }</td>
				                                        <td>
				                                        	<c:if test="${campaign.use_yn eq 'Y' }">사용</c:if>
				                                        	<c:if test="${campaign.use_yn eq 'N' }">미사용</c:if>
				                                        </td>
				                                        <td>
				                                            <a href="#" class="btn btn-info btnDetail btn-sm" data-code="${campaign.seq }" data-pidx="${campaign.seq }">보기</a>
				                                        </td>
				                                    </tr>
	                                			</c:forEach>
	                                		</c:when>
	                        ...
	                        <div class="text-center paging">
	                        	<p class="txt_count text-orange"><b>등록된 캠페인 : ${totalCount }</b></p>
...
$(document).ready(function(){
    ...
    */
    ...
    $(".btnDetail").on("click",function(){
    	...
    	movePage(data,"campaignDetail.tsis");
    });
