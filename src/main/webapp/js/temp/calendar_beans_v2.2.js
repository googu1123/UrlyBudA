...
function CalAddCss(){
		...
		cssHtml += ".calLayout{	border:1px solid #DDDDDD; background-color:#ebebeb;	position:absolute;padding:2px;display:none;z-index:9999	}";			
		...
		cssHtml += ".calDay{background-color: #fff;border:1px solid #CBC7BD;	width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;}";
		cssHtml += ".calSunDay{	background-color: #fff;border:1px solid #CBC7BD;	width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:red;}";
		cssHtml += ".calSatDay{ background-color: #fff;border:1px solid #CBC7BD;	width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;	color:blue;}";
		cssHtml += ".calOtherDay{background-color: #fff;	border:1px solid #CBC7BD;width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:#CBC7BD;}";
		cssHtml += ".dayToday{background-color: #00aeef;border:1px solid #CBC7BD;font-weight:bold;font-weight:bold;width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:white;	}";
		cssHtml += ".calSelDay{	background-color: #ed1c24;border:1px solid #CBC7BD;font-weight:bold;width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;text-decoration:underline;color:white;}";
		...
		cssHtml += ".MonToday{font-size:8pt;width:25px;	height:15px;background-color: #00aeef;border:1px solid #CBC7BD;	font-weight:bold;padding:4px 4px 0px 3px;color:white;cursor:pointer;}";
		cssHtml += ".selMon{font-size:8pt;width:25px;height:15px;background-color: #ed1c24;border:1px solid #CBC7BD;font-weight:bold;padding:4px 4px 0px 3px;text-decoration:underline;color:white;}";
		cssHtml += ".calOnDay{background-color: #000;border:1px solid #4B22C0;float:left;cursor:pointer;color:white;}";
...
function initCal(obj){
	...
	// 최소년도, 최대년도 
	var minYear 	= obj.min == null ? "2013" : obj.min;
	...
	$("#"+obj.id + ",#"+ calId + "Icon").click(function(event){					
		...
		if( calType == "day"){
			makeCal(calId,dates,splitKey,setId,url);
		}else{
			makeCalMon(calId,dates,splitKey,setId,url);
		}
...
//클릭 이벤트
function calClickEvent(id,setId,url){
	$("#" + id + " .calOtherDay,.calDay,.calSunDay,.calSatDay,.MonToday,.selMon,.dayEvMon").click(function(event) {
        ...
        if($("#day_type").val()=="M"){
        	//alert(this.id.replace(id,"") == "");
        	var month =  this.id.replace(id,"").split("-")[1];
        	if(month == "02"){
        		$("#"+setId).val(this.id.replace(id,"")+"-28");
        	} else {
        		$("#"+setId).val(this.id.replace(id,"")+"-30");
        	}
        } else {
        	$("#"+setId).val(this.id.replace(id,""));
        }
        if(url != null){
		...
		var index = $("#" + id + " .calOtherDay,.calDay,.calSunDay,.calSatDay,.MonToday,.selMon,.dayEvMon").index(this);
		if($("#day_type").val()=="D"){
			$(this).addClass("calOnDay");
		} else if($("#day_type").val()=="W"){
			for(var i = index-6 ; i <= (index) ;i++){
				$("#" + id + " .calOtherDay,.calDay,.calSunDay,.calSatDay,.MonToday,.selMon,.dayEvMon").eq(i).addClass("calOnDay");
			}
		} else {
			$(this).addClass("calOnDay");
		}
		...
		if($("#day_type").val()=="D"){
			...
			for(var i = index-6 ; i <= (index) ;i++){
				$("#" + id + " .calOtherDay,.calDay,.calSunDay,.calSatDay,.MonToday,.selMon,.dayEvMon").eq(i).removeClass("calOnDay");
			}
		} else {
			$(this).removeClass("calOnDay");
		}
...
// 날짜변경(달변경) 달력내용 그리기
function makeCal(id,dates,splitKey,setId,url){				
	...
	calClickEvent(id,setId,url);
}

function makeCalMon(id,dates,splitKey,setId,url){
